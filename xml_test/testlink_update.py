...
help_msg = '''app template namelist outfile
'''
xmlheader = '''<?xml version='1.0' encoding='UTF-8'?><testcases></testcases>
'''
if __name__ == "__main__" :
    if len(sys.argv) != 4 :
        print help_msg
        exit(0)
    dom1 = minidom.parseString(xmlheader)
    
    dom2 = minidom.parse(sys.argv[1])
    
    names = open(sys.argv[2],'r').readlines()
    out = open(sys.argv[3],'w')

    for item  in names :
        dom2.childNodes[0].setAttribute("name",item)
